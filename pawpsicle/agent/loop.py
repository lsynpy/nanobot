...
from pawpsicle.agent.tools.cron import CronTool
from pawpsicle.agent.tools.filesystem import EditFileTool, ListDirTool, ReadFileTool, WriteFileTool
from pawpsicle.agent.tools.message import MessageTool
from pawpsicle.agent.tools.registry import ToolRegistry
from pawpsicle.agent.tools.shell import ExecTool
from pawpsicle.agent.tools.spawn import SpawnTool
...
if TYPE_CHECKING:
    from pawpsicle.config.schema import ChannelsConfig, ExecToolConfig
...
class AgentLoop:
    ...
    async def _connect_mcp(self) -> None:
        ...
        from pawpsicle.agent.tools.mcp import connect_mcp_servers
        ...
        for name in ("message", "spawn", "cron"):
            if tool := self.tools.get(name):
                if hasattr(tool, "set_context"):
                    fn = tool.set_context
                    if name == "message":
                        fn(channel, chat_id, message_id)  # type: ignore[call-arg]
                    else:
                        fn(channel, chat_id)  # type: ignore[call-arg]
